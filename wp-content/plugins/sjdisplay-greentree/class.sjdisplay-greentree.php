...
class SJDisplay_Greentree
{
    ...
    public static function create_order_xml($order_data)
    {
        ...
        $instructions = 'Contact: ' . $order_data['contact'];
        ...
        $order_xml = '<?xml version="1.0" encoding="UTF-8"?>
                        <SOSalesOrder>
                            ...
                            <AddressName>' . htmlspecialchars($order_data['address_name']) . '</AddressName>
                            <StandardText>' . htmlspecialchars($standard_text) . '</StandardText>
                            <DeliveryAddress>
                                <Contact>' . $order_data['contact'] . '</Contact>
                                <Address1>' . htmlspecialchars($order_data['address_1']) . '</Address1>
                                <Address2>' . htmlspecialchars($order_data['address_2']) . '</Address2>
                                <Address3>' . htmlspecialchars($order_data['address_3']) . '</Address3>
                                <Instructions>' . htmlspecialchars($instructions) . '</Instructions>
                                <Suburb>' . $order_data['suburb'] . '</Suburb>
    ...
    public static function create_receipt_xml($receipt_data)
    {
        ...
        $receipt_xml = '<?xml version="1.0" encoding="UTF-8"?>
                        <Receipt>
                            ...
                            <NetAmount>' . $receipt_data['order']->get_total() . '</NetAmount>
                            <TaxAmount>' . $receipt_data['order']->get_total_tax() . '</TaxAmount>
                            <IsCurrencyRateMultiply>false</IsCurrencyRateMultiply>
                            <Branch>03</Branch>
    ...
    */
    ...
    /*
     * Function to return area_manager user for a store
     */
    ...
    {
        $order_greentree_reference_id = get_post_meta($order->ID, 'order_greentree_reference_id', true);
        $order_greentree_status = get_post_meta($order->ID, 'order_greentree_status', true);
        $order_greentree_modified_timestamp = get_post_meta($order->ID, 'order_greentree_modified_timestamp', true);
        $order_greentree_modified_user = get_post_meta($order->ID, 'order_greentree_modified_user', true);

        if ($verbose) {
            echo '   Woo order ID: ' . $order->ID . '<br>';
            echo '   Woo order status: ' . $order->post_status . '<br>';
        ...
        $delivery_address = array(
            'first_name' => $split_deliver_address_name[0],
            'last_name' => $split_deliver_address_name[1],
        ...
        if ($live_order_data->LineItems->LineItem) {
            foreach ($live_order_data->LineItems->LineItem as $line_item) {
                ...
                //skip for Z-FREIGHT item
                if ($line_item_sku == 'Z-FREIGHT') {
                    if ($verbose) {
                        echo '      -- Skipping Z-FREIGHT line item...<br>';
                    }
                    continue;
                }
                ...
                if ($item_found) {
                    ...
                    if ($order_items[$item_found]->get_quantity() == $line_item_qty) {
                        if ($verbose) {
        ...
        update_post_meta($order->ID, 'order_greentree_status', strval($live_order_data->Status));
        update_post_meta($order->ID, 'order_greentree_modified_timestamp', strval($live_order_data->ModifiedTimeStamp));
        update_post_meta($order->ID, 'order_greentree_modified_user', strval($live_order_data->ModifiedUser));

        //current woo status as default
        $woo_order_status = $order->post_status;
        ...
        if ($woo_order_status == 'completed' || $woo_order_status == 'processing') {
            ...
            update_post_meta($order->ID, 'order_greentree_invoice', json_encode($order_invoice_data));
        }
        ...
        $stored_order_invoices = json_decode(get_post_meta($order->ID, 'order_greentree_invoice', true));
        if ($stored_order_invoices) {
            ...
            update_post_meta($order->ID, 'order_greentree_packing_slip', json_encode($packing_slip_data));
        }
        ...
        //send notification of update to user if required
        if ($update_notification_required) {
            SJDisplay_Greentree_Woo::send_store_manager_order_update_notification($order->ID, 'updated');
        }
    ...
    public static function import_customer($customer, $parent_customer, $verbose = false, $script)
    {
        ...
        if ($user_data_array['user_data_web_username']['value'] != '') {
            ...
            if (!$store_manager_user_id and email_exists($user_data_array['user_data_web_username']['value']) == false) {
                ...
                $store_manager->add_role('store_manager');
            } else {
                if ($verbose) {
        ...
        foreach ($approval_users as $approval_user) {
            if ($approval_user != '') {
                ...
                if (!$area_manager_user_id and email_exists($approval_user) == false) {
                    ...
                    $area_manager->set_role('editor');
                    $area_manager->add_role('area_manager');
                    $area_manager->add_role('shop_manager');
                } else {
                    if ($verbose) {
